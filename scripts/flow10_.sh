...
python3 -m mlonmcu.cli.main flow run ${BENCHMARKS[@]} --target $TARGET -c run.export_optional=1 -c $TARGET.abi=$ABI -c mlif.debug_symbols=1 -v -c mlif.toolchain=llvm --label $LABEL-compare -c etissvp.script=$WORK/docker/etiss_install/bin/run_helper.sh -c etiss.cpu_arch=$CORE_NAME -c $TARGET.print_outputs=1 -c llvm.install_dir=$WORK/docker/llvm_install --config-gen $TARGET.arch=$ARCH --config-gen $TARGET.arch=${ARCH}_xisaac --post config2cols -c config2cols.limit=$TARGET.arch --post rename_cols -c rename_cols.mapping="{'config_$TARGET.arch': 'Arch'}" --post compare_rows -c compare_rows.to_compare="Run Instructions" --parallel 4 -c mlif.unroll_loops=$UNROLL -c mlif.optimize=$OPTIMIZE
...
python3 -m mlonmcu.cli.main flow compile ${BENCHMARKS[@]} --target $TARGET -c run.export_optional=1 -c $TARGET.abi=$ABI -c mlif.debug_symbols=1 -v -c mlif.toolchain=llvm --label $LABEL-compare-mem -c etissvp.script=$WORK/docker/etiss_install/bin/run_helper.sh -c etiss.cpu_arch=$CORE_NAME -c $TARGET.print_outputs=1 -c llvm.install_dir=$WORK/docker/llvm_install --config-gen etiss.arch=$ARCH --config-gen etiss.arch=${ARCH}_xisaac --post config2cols -c config2cols.limit=$TARGET.arch --post rename_cols -c rename_cols.mapping="{'config_$TARGET.arch': 'Arch'}" --post compare_rows -c compare_rows.to_compare="ROM code" -c mlif.strip_strings=1 --parallel 4 -c mlif.unroll_loops=$UNROLL -c mlif.optimize=$OPTIMIZE
