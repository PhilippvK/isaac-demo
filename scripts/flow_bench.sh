...
then
    ...
    $DOCKER_PREFIX docker run -i --rm -e MLONMCU_HOME=$MLONMCU_HOME -v $MLONMCU_HOME:$MLONMCU_HOME -v $(pwd):$(pwd) --workdir $(pwd) $MLONMCU_MIN_IMAGE $MLONMCU_ARGS
elif [[ "$USE_MLONMCU_DOCKER" == "1" ]]
then
    MLONMCU_IMAGE=${MLONMCU_IMAGE:-philippvk/isaac-quickstart-mlonmcu:latest}
    $DOCKER_PREFIX docker run -i --rm -v $(pwd):$(pwd) --workdir $(pwd) $MLONMCU_IMAGE $MLONMCU_ARGS
else
