...
then
    ...
    $DOCKER_PREFIX docker run -i --rm -v $(pwd):$(pwd) $SEAL5_IMAGE $SEAL5_DEST_DIR $CDSL_FILES $CFG_FILES
...
then
    ...
    $DOCKER_PREFIX docker run -i --rm -v $(pwd):$(pwd) $ETISS_IMAGE $ETISS_DEST_DIR $GEN_DIR/$ETISS_CORE_NAME.core_desc
fi
